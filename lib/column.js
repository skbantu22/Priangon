...
export const DT_PRODUCT_COLUMN = [
  {
    ...
    Cell: ({ row }) => (
      <div className="flex items-center gap-3">
        <Image
          src={row.original.image || "/assets/placeholder.png"}
...

  {
    ...
    enableSorting: false,
    Cell: ({ row }) => {
      const label = warrantyLabel(row.original.warranty);
      return (
        <div className="leading-tight">
          <span className={label ? "text-emerald-700" : "text-gray-400"}>
            {label || "No warranty"}
          </span>
          {row.original.trackSerial && (
            <span className="block text-[11px] text-gray-500">IMEI / Serial required</span>
          )}
        </div>
      );
    },
