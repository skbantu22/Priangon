...
async function figures(ctx) {
  ...
  const [saleTotal, cost, expenses, purchases, returns, posReceived, dueReceived, purchasePaid, supplierPaid, discount, vat, saleReturns] = await Promise.all([
...
export const REPORTS = {
  ...
  "profit-loss-report": {
    ...
    run: async (ctx) => {
      ...
      const c = [["Expense", f.expenses], ["Total Expense", f.expenses], [net >= 0 ? "Net Profit" : "Net Loss", net]];
      return {
        columns: [
          ["a", "Item", "text"],
          ["a_amt", "Amount", "rate"],
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        rows: [
          ...
          { item: "Net Profit", amount: round(f.sales - f.saleReturns - f.cost - f.expenses) },
        ],
      };
    },
  },
