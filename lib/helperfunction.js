...
const fieldLabel = (path) => {
  const words = String(path || "field").split(".").pop().replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase();
  return words.charAt(0).toUpperCase() + words.slice(1);
};

/**
 * Says which fields a save failed on, instead of a bare "Validation error",
 * so the person filling the form knows what to fix.
 */
export const validationMessage = (error) => {
  const problems = Object.values(error?.errors || {}).map((e) =>
    e?.kind === "required" ? `${fieldLabel(e.path)} is required` : e?.message || `${fieldLabel(e?.path)} is not valid`,
  );
  return problems.length ? `Check these: ${problems.join("; ")}` : "Validation error.";
};

/** The same for a zod check: "Name: Name must be at least 2 characters; ..." */
export const zodMessage = (zodError) => {
  const issues = zodError?.issues || [];
  if (!issues.length) return "Invalid or missing fields.";
  return `Check these: ${issues
    .slice(0, 4)
    .map((i) => (i.path?.length ? `${fieldLabel(i.path.join("."))}: ${i.message}` : i.message))
    .join("; ")}`;
};
...
export const catchError = (error, customMessage) => {
  ...
  if (error?.name === "ValidationError") {
    ...
    return response(false, 400, customMessage || validationMessage(error), data);
  }
