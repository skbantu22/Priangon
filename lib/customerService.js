 ...
 * twice).
 *
 *   due = total − paid − dismiss − advance − returned + paid out to the customer
 *
 * Goods brought back (a sales return) come off the due; cash handed back
 * for them is a "pay" payment and adds back in.
 *
...
export async function customerBalances(customers) {
  ...
  for (const customer of customers) {
    ...
    balances.set(id, {
      ...
      due: round(total - paid - dismiss - advance - returned + paidOut),
    });
  }
...
export async function customerLedger(customer, { start, end } = {}) {
  ...
  const [orders, payments, returns] = await Promise.all([
    ...
    SaleReturn.find({ customerId: customer._id, deletedAt: null }).select("returnNumber returnDate total orderNumber note").lean(),
  ]);
  ...
  for (const ret of returns) {
    lines.push({
      ...
      note: [ret.note, `From ${ret.orderNumber}`].filter(Boolean).join(" · "),
  ...
  return {
    ...
    summary: {
      ...
      returned: round(-inPeriod("Sale Return")),
