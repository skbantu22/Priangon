...
export default function SupplierPaymentList({ type, title }) {
  ...
  const head = ["SL", "Date", "Invoice No", "Supplier", "Amount", "Method", byLabel, "Note"];
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get("/api/supplier-payments", { params: params({ page, limit }) });
  ...
  const remove = async (row) => {
    ...
    try {
      const { data } = await axios.delete(`/api/supplier-payments/${row._id}`);
  ...
  const withAllRows = async (handle) => {
    ...
    try {
      const { data } = await axios.get("/api/supplier-payments", { params: params({ limit: "all" }) });
            ...
            <table className="w-full min-w-[760px] border-collapse text-sm">
              <thead>
                <tr className={theadRow}>
                  {["SL", "Date", "Invoice No", "Supplier", "Amount", byLabel, "Action"].map((column) => (
                ...
                {!loading &&
                  rows.map((row, index) => (
                    <tr key={row._id}>
                      ...
                      <td className={tdClass}>{row.supplierId?.name}</td>
          ...
          {viewing && (
            <div className="space-y-4 text-sm">
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5">
                {[
                  ["Supplier", viewing.supplierId?.name],
                  ["Mobile", viewing.supplierId?.phone],
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {viewing.allocations.map((share) => (
                      <tr key={share.purchaseId}>
                        <td className={tdClass}>{share.purchaseNumber}</td>
