...
import { ADMIN_SUPPLIER_LEDGER } from "@/Route/Adminpannelroute";
...
/** A supplier's account statement */
export default function SupplierLedgerPage({ params }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) => {
        if (!cancelled) {
          showToast("error", error.response?.data?.message || "Could not load ledger");
        }
      });
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((row) =>
      [row.type, row.invoiceNo, row.note, fmtDate(row.date), money(row.amount)]
        .join(" ")
        .toLowerCase()
        .includes(needle),
    );
  ...
  const title = `Supplier Ledger — ${data?.supplier.name || ""}`;
  ...
  return (
    <div>
      ...
      >
        ...
        <select
          value={id}
          onChange={(event) => event.target.value && router.push(ADMIN_SUPPLIER_LEDGER(event.target.value))}
          className={`${inputClass} !w-60`}
          aria-label="Supplier"
        >
          {!suppliers.some((item) => item._id === id) && data && (
            <option value={id}>{data.supplier.name}</option>
          )}
          {suppliers.map((item) => (
        ...
        <>
          <div className="grid gap-4 lg:grid-cols-2">
            <section className="rounded-[10px] border border-t-[4px] border-[#e6ebf1] border-t-[#16a34a] bg-white p-[18px] dark:border-border dark:border-t-[#16a34a] dark:bg-card">
                ...
                <h2 className="mt-3 text-lg font-bold">{data.supplier.name}</h2>
                {data.supplier.companyName && (
                  <p className="text-sm text-muted-foreground">{data.supplier.companyName}</p>
                )}
                {data.supplier.phone && (
                  <a href={`tel:${data.supplier.phone}`} className="mt-2 block text-sm">
                    📞 {data.supplier.phone}
                  </a>
                )}
                {data.supplier.address && (
                  <p className="mt-2 text-sm text-muted-foreground">{data.supplier.address}</p>
                )}
            </section>
            ...
            <section className="rounded-[10px] border border-t-[4px] border-[#e6ebf1] border-t-[#16a34a] bg-white p-[18px] dark:border-border dark:border-t-[#16a34a] dark:bg-card">
                ...
                {[
                  ...
                  ["Total paid", s.totalPaid, "text-green-600"],
                  ["Due dismiss", s.dismiss],
                  ["Received back", s.received],
                  ["Advance", s.advance],
                ...
                <div className="flex justify-between pt-2.5 font-bold">
                  ...
                  <span className={`tabular-nums ${s.due > 0 ? "text-red-600" : "text-green-600"}`}>
                    ৳ {money(s.due)}
                  </span>
                </div>
            </section>
          ...
          <section className="mt-[18px] rounded-[10px] border border-[#e6ebf1] bg-white p-[18px] dark:border-border dark:bg-card">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <ExportButtons
                  ...
                  onExcel={() => exportExcel("Supplier-Ledger.xlsx", HEAD, exportBody())}
                  onPrint={() => {
                    ...
                    {rows.map((row) => (
                      <tr key={row.sl} className="hover:bg-muted/50">
                        ...
                        <td className={tdClass}>
                          {row.type}
                          {row.method && (
                            <span className="block text-xs text-muted-foreground">{methodLabel(row.method)}</span>
                          )}
                        </td>
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                A + amount adds to what the shop owes; a − amount takes it down.
              </p>
          </section>
        </>
      )}
