...
/** Expenses → Expense Type: create/edit on the left, the list on the right, like 360 */
export default function ExpenseTypePage() {
  ...
  const load = async () => {
    try {
      const { data } = await axios.get("/api/expense-category");

      if (data.success) setTypes(data.data);
      else showToast("error", data.message || "Could not load expense types");
    } catch (err) {
      showToast("error", err.response?.data?.message || "Could not load expense types");
      setTypes([]);
    }
  };
  ...
  const save = async (event) => {
    ...
    try {
      const { data } = editing
        ? await axios.put(`/api/expense-category/update/${editing._id}`, { name })
        : await axios.post("/api/expense-category/create", { name });
      ...
      setError(err.response?.data?.message || "Could not save expense type");
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (row) => {
    ...
    try {
      const { data } = await axios.delete(`/api/expense-category/delete/${row._id}`);
      ...
      showToast("error", err.response?.data?.message || "Could not delete expense type");
    }
  };
    ...
    <div className="grid grid-cols-1 items-start gap-[16px] lg:grid-cols-[minmax(320px,1fr)_2fr] lg:gap-[24px]">
      <form onSubmit={save} noValidate className={`${card} px-[16px] py-[20px] sm:px-[25px] sm:py-[28px] lg:sticky lg:top-[84px]`}>
        <div className="mb-[22px] flex items-center justify-between gap-3">
          <h1 className={heading}>{editing ? "Update Expense Type" : "Create Expense Type"}</h1>
          {editing && (
        ...
        <label htmlFor="et-name" className="mb-[10px] block text-[16px] font-medium">
          Expense Type <span className="text-[#f05252]">*</span>
        </label>
        <input
          id="et-name"
          ref={nameRef}
          ...
          maxLength={150}
          autoComplete="off"
          placeholder="Expense Type, e.g. Electricity Bill"
          onChange={(event) => {
      ...
      <section className={`${card} border-t-[3px] border-t-[#00801a] px-[16px] py-[18px] sm:px-[25px] sm:py-[22px] dark:border-t-[#00801a]`}>
        <h2 className={heading}>Expense Type List</h2>
                  ...
                  <td colSpan={3} className={cell}>
                    <div className="flex flex-col items-center gap-[6px] py-[26px] text-center">
                      ...
                      <p className="m-0 text-[15px]">
                        {term ? `No expense type matches “${term}”` : "No data available in table"}
                      </p>
                      {!term && <p className="m-0 text-[13px] text-[#8a939c]">Add your first expense type with the form.</p>}
                    </div>
                  </td>
                </tr>
              )}
