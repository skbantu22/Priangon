...
/** Purchases → Return Type (damaged, wrong item, expired…): create/edit on the left, the list on the right */
export default function PurchaseReturnTypePage() {
  ...
  const load = async () => {
    try {
      const { data } = await axios.get("/api/purchase-return-types");

      if (data.success) setTypes(data.data);
      else showToast("error", data.message || "Could not load return types");
    } catch (err) {
      showToast("error", err.response?.data?.message || "Could not load return types");
      setTypes([]);
    }
  };
  ...
  const save = async (event) => {
    ...
    try {
      const { data } = editing
        ? await axios.put(`/api/purchase-return-types/${editing._id}`, { name })
        : await axios.post("/api/purchase-return-types", { name });
      ...
      setError(err.response?.data?.message || "Could not save return type");
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (row) => {
    ...
    try {
      const { data } = await axios.delete(`/api/purchase-return-types/${row._id}`);
      ...
      showToast("error", err.response?.data?.message || "Could not delete return type");
    }
  };
    ...
    <div className="grid grid-cols-1 items-start gap-[16px] lg:grid-cols-[minmax(320px,1fr)_2fr] lg:gap-[24px]">
      <form onSubmit={save} noValidate className={`${card} px-[16px] py-[20px] sm:px-[25px] sm:py-[28px] lg:sticky lg:top-[84px]`}>
        <div className="mb-[22px] flex items-center justify-between gap-3">
          <h1 className={heading}>{editing ? "Update Return Type" : "Create Return Type"}</h1>
          {editing && (
        ...
        <label htmlFor="rt-name" className="mb-[10px] block text-[16px] font-medium">
          Return Type <span className="text-[#f05252]">*</span>
        </label>
        <input
          id="rt-name"
          ref={nameRef}
          ...
          maxLength={150}
          autoComplete="off"
          placeholder="Return Type, e.g. Damaged"
          onChange={(event) => {
      ...
      <section className={`${card} border-t-[3px] border-t-[#00801a] px-[16px] py-[18px] sm:px-[25px] sm:py-[22px] dark:border-t-[#00801a]`}>
        <h2 className={heading}>Return Type List</h2>
        ...
        <div className="mt-[16px] flex flex-col gap-[12px] sm:flex-row sm:items-center sm:justify-between">
          ...
          <label className="relative block sm:w-[250px]">
            ...
            <input
              ...
              placeholder="Search"
              aria-label="Search return types"
                  ...
                  <td colSpan={3} className={cell}>
                    <div className="flex flex-col items-center gap-[6px] py-[26px] text-center">
                      ...
                      <p className="m-0 text-[15px]">
                        {term ? `No return type matches “${term}”` : "No data available in table"}
                      </p>
                      {!term && <p className="m-0 text-[13px] text-[#8a939c]">Add your first return type with the form.</p>}
                    </div>
                  </td>
                </tr>
              )}
